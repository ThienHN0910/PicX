                    ...
                    <div className="relative z-10">
                        <div className="flex justify-between items-center mb-8">
                            ...
                            <div className="text-right">
                                {order.status === 'Paid' ? (
                                    <span className="inline-flex items-center gap-2 px-6 py-3 bg-green-500 text-white rounded-full text-lg font-semibold shadow-lg">
                                        <CreditCard className="h-5 w-5" />
                                        Paid
                                    </span>
                                ) : order.status === 'Pending' ? (
                                    <span className="inline-flex items-center gap-2 px-6 py-3 bg-yellow-400 text-white rounded-full text-lg font-semibold shadow-lg">
                                        <CreditCard className="h-5 w-5" />
                                        Pending
                                    </span>
                                ) : order.status === 'Cancelled' ? (
                                    <span className="inline-flex items-center gap-2 px-6 py-3 bg-red-500 text-white rounded-full text-lg font-semibold shadow-lg">
                                        <CreditCard className="h-5 w-5" />
                                        Cancelled
                                    </span>
                                ) : (
                                    <span className="inline-flex items-center gap-2 px-6 py-3 bg-gray-400 text-white rounded-full text-lg font-semibold shadow-lg">
                                        <CreditCard className="h-5 w-5" />
                            ...
                            <div key={item.productId} className="group bg-gradient-to-r from-slate-50 to-gray-50 rounded-2xl p-6 border border-slate-200 hover:shadow-lg transition-all duration-300">
                                <div className="flex items-center gap-6">
                                    ...
                                    <div className="text-right space-y-2">
                                        ...
                                        {/* Action Buttons: Thanh toán nếu Pending, Download nếu Paid */}
                                        {order.status === 'Pending' ? (
                                            <button
                                                className="w-full bg-yellow-400 hover:bg-yellow-500 text-white px-4 py-2.5 rounded-lg shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-[1.02] active:scale-[0.98] group"
                                                onClick={() => navigate(`/payment/${order.orderId}`)}
                                            >
                                                <div className="flex items-center justify-center space-x-1.5">
                                                    <CreditCard className="w-4 h-4" />
                                                    <span className="text-sm font-semibold">Thanh toán ngay</span>
                                                </div>
                                            </button>
                                        ) : order.status === 'Paid' ? (
                                            <>
